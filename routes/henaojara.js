...
exports.SearchHenaojara = async function (query, genreArr = undefined, url = undefined, page = undefined, gottenItems = 0) {
  ...
  const henaojaraURL = (url) ? url
    : `${encodeURIComponent(HENAOJARA_BASE)}%2Fanimes%3F${(query) ? "search%3D" + encodeURIComponent(query) + "%26" : ""}${(genreArr) ? encodeURIComponent("genero%3D" + genreArr.join("%2C")) : ""}${(page) ? "%26page%3D" + page : ""}`
